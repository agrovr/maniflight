function hasTestCommand(snapshot) {
    return snapshot.manifests.some((manifest) => manifest.scripts.some((script) => /^(test|test:|check$|verify$)/u.test(script)));
}
export const manifestPresent = (snapshot) => {
    ...
    return {
        ...
        description: "A machine-readable project manifest makes the repository reproducible and navigable.",
        status: !applicable ? "not_applicable" : valid.length > 0 ? "pass" : "fail",
        severity: "medium",
        weight: 3,
        evidence: valid.length > 0
            ? valid.map((manifest) => ({
                kind: "present",
...
export const sourceBoundary = (snapshot) => {
    ...
    return {
        ...
        description: "Production code is grouped under a recognizable application, package, or source directory.",
...
export const testCapability = (snapshot) => {
    const applicable = snapshot.facts.sourceFileCount > 0;
...
export const typescriptStrict = (snapshot) => {
    const facts = snapshot.facts.typescript;
    const status = !facts.used
        ? "not_applicable"
        : !facts.configPath
            ? "fail"
                ...
                ? [
                    {
                        ...
                        message: facts.strict === undefined
...
export const packageEntrypoints = (snapshot) => {
    const entrypoints = snapshot.manifests
        .filter((manifest) => manifest.kind === "npm")
        .flatMap((manifest) => manifest.entrypoints.map((entrypoint) => ({ ...entrypoint, manifest: manifest.path })));
    const existing = entrypoints.filter((entrypoint) => entrypoint.exists);
    const status = entrypoints.length === 0
        ? "not_applicable"
    ...
    return {
        ...
        remediation: "Correct stale entrypoint paths or ensure required build outputs exist before publishing.",
    };
};
