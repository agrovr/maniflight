...
function documentation(snapshot, name) {
...
function hasWriteAllObject(permissions) {
    if (!permissions || typeof permissions === "string")
        return false;
    const values = Object.values(permissions);
    return values.length > 5 && values.every((value) => value === "write");
}
export const securityPolicy = (snapshot) => {
...
export const lockfilePresent = (snapshot) => {
    const applicable = snapshot.manifests.length > 0;
    const dependencies = snapshot.manifests.reduce((total, manifest) => total + manifest.dependencyCount + manifest.developmentDependencyCount, 0);
    return {
        ...
        severity: "high",
        weight: 3,
        evidence: snapshot.facts.lockfilePaths.length > 0
...
export const dependencyUpdates = (snapshot) => ({
    ...
    status: snapshot.manifests.length === 0
        ? "not_applicable"
    ...
    severity: "medium",
    weight: 2,
    evidence: snapshot.facts.dependencyUpdatePaths.length > 0
...
export const workflowPermissions = (snapshot) => {
    ...
    for (const workflow of snapshot.workflows) {
        ...
        if (workflowRisk === "missing" &&
            (workflow.jobs.length === 0 || workflow.jobs.some((job) => job.permissions === undefined))) {
            missing += 1;
        }
        if (workflowRisk === "write-all" || hasWriteAllObject(workflow.permissions))
            broad += 1;
    ...
    const status = broad > 0 ? "fail" : missing > 0 ? "warn" : "pass";
    return {
        ruleId: "security/workflow-permissions",
        domain: "security",
        title: "Workflow token permissions",
...
export const actionReferencePinned = (snapshot) => {
    const references = snapshot.workflows.flatMap((workflow) => workflow.jobs.flatMap((job) => job.actionReferences.map((reference) => ({ reference, path: workflow.path }))));
    ...
    return {
        ...
        description: "External Actions and containers are pinned to immutable commit or digest references.",
        ...
        remediation: "Pin third-party Actions to a full 40-character commit SHA and containers to a digest.",
        documentationUrl: SECURE_USE,
    };
};
export const pullRequestTargetCheckout = (snapshot) => {
    const risky = snapshot.workflows.filter((workflow) => workflow.triggers.includes("pull_request_target") &&
        workflow.jobs.some((job) => job.actionReferences.some((reference) => reference.repository?.toLowerCase().startsWith("actions/checkout"))));
    return {
        ruleId: "security/pull-request-target-checkout",
        domain: "security",
        title: "Privileged pull request checkout",
        description: "pull_request_target workflows do not combine privileged context with a source checkout.",
        ...
        evidence: risky.length > 0
            ...
            : snapshot.workflows.length > 0
                ...
                : [],
        remediation: "Use pull_request for untrusted code, or avoid checking out pull request code in privileged jobs.",
        documentationUrl: SECURE_USE,
    };
};
export const untrustedContextInRun = (snapshot) => {
    const expressions = snapshot.workflows.flatMap((workflow) => workflow.jobs.flatMap((job) => job.untrustedExpressions));
    return {
        ruleId: "security/untrusted-context-in-run",
        domain: "security",
        title: "Untrusted context in shell",
        description: "Inline shell scripts do not interpolate attacker-controlled GitHub context directly.",
        status: snapshot.workflows.length === 0 ? "not_applicable" : expressions.length > 0 ? "fail" : "pass",
        severity: "high",
        weight: 4,
        evidence: expressions.length > 0
            ...
            : snapshot.workflows.length > 0
                ...
                : [],
        remediation: "Pass untrusted values through an environment variable and quote them in the shell.",
        documentationUrl: "https://docs.github.com/en/actions/concepts/security/script-injections",
    };
};
export const sensitiveFilename = (snapshot) => ({
    ruleId: "security/sensitive-filename",
    domain: "security",
    title: "Sensitive filenames",
    description: "The checkout does not contain common private-key, credential, secret, or environment filenames.",
    status: snapshot.facts.sensitiveFilePaths.length > 0 ? "fail" : "pass",
    severity: "high",
    weight: 3,
    evidence: snapshot.facts.sensitiveFilePaths.length > 0
    ...
    remediation: "Remove sensitive files from version control, rotate exposed credentials, and add safe ignore rules.",
});
