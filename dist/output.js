...
const checkResultSchema = z
    ...
    .strict();
function domainResultSchema(domain) {
    return z
        .object({
...
async function assertBaselinePath(path, allowedRoot) {
    if (!allowedRoot)
        return;
...
async function readBoundedBaseline(path, allowedRoot) {
    let beforeOpen;
    try {
    ...
    if (beforeOpen.size > BASELINE_REPORT_MAX_BYTES) {
        throw new Error(`Baseline report exceeds the ${BASELINE_REPORT_MAX_BYTES}-byte size limit: ${path}`);
    }
...
function parseBaselineReport(contents, path) {
    let value;
    try {
    ...
    if (typeof value === "object" &&
        ...
        throw new Error(`Unsupported baseline report schemaVersion in ${JSON.stringify(path)}; expected "1.0"`);
    }
    ...
    if (!parsed.success) {
        const details = parsed.error.issues
            ...
            .join("; ");
        throw new Error(`Baseline report does not match schema 1.0: ${JSON.stringify(path)}: ${details}`);
    }
...
export async function loadBaselineReport(reportPath, allowedRoot) {
...
async function writeComparisonArtifactInDirectory(comparison, directory) {
    const path = resolve(directory, "comparison.json");
    await writeArtifact(path, `${JSON.stringify(comparison, null, 2)}\n`);
    return path;
}
export async function writeComparisonArtifact(comparison, outputDirectory, allowedRoot) {
...
export async function writeReportArtifacts(report, outputDirectory, allowedRoot, comparison) {
    const directory = await prepareOutputDirectory(outputDirectory, allowedRoot);
    ...
    if (comparison)
        writes.push(writeComparisonArtifactInDirectory(comparison, directory));
