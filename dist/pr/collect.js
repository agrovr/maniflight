...
function safeSource(id, status, detail) {
...
function statusOf(error) {
    ...
    if (direct !== undefined && direct >= 100 && direct <= 599)
        return direct;
...
function hasNextPage(response) {
    ...
    for (const [name, value] of Object.entries(response.headers)) {
        if (name.toLowerCase() !== "link" || typeof value !== "string")
            continue;
        if (/(?:^|,)\s*<[^>]+>\s*;[^,]*\brel=["']?next["']?/iu.test(value))
            return true;
    }
    return false;
}
...
function parseSubject(data, reference) {
    ...
    if (!base || !head || (state !== "open" && state !== "closed"))
        return undefined;
...
function unwrapGraphqlResult(value) {
    if (!isRecord(value))
        return undefined;
...
function parseGraphqlAggregate(value) {
    ...
    if (!hasMergeState || !hasReviewDecision)
        partial = true;
    const mergeState = optionalSanitizedText(pullRequest.mergeStateStatus, 80)?.toLowerCase() ?? null;
    const reviewDecision = optionalSanitizedText(pullRequest.reviewDecision, 80)?.toLowerCase() ?? null;
    ...
    if (!pageInfo || typeof pageInfo.hasNextPage !== "boolean")
        partial = true;
    if (pageInfo?.hasNextPage === true || total > threads.nodes.length)
        partial = true;
    return {
        mergeState,
        reviewDecision,
...
function parseReviews(data, fallbackUrl) {
...
function parseCheckRuns(data, fallbackUrl) {
    if (!isRecord(data) || !Array.isArray(data.check_runs))
        return null;
    let malformed = false;
    const values = [];
...
function parseCommitStatuses(data, fallbackUrl) {
...
function parseWorkflowRuns(data, fallbackUrl) {
...
function booleanAggregate(values) {
    if (values.includes(true))
        return true;
    return values.includes(false) ? false : null;
}
function parseBranchPolicy(data) {
    ...
    let malformed = false;
    const approvals = [];
    const codeOwnerValues = [];
    const lastPushValues = [];
    const threadValues = [];
    const upToDateValues = [];
    const requiredChecks = [];
    const requiredCheckApps = [];
    let relevantRules = 0;
    for (const item of rules) {
        ...
        if (type === "pull_request") {
            ...
            codeOwnerValues.push(typeof parameters.require_code_owner_review === "boolean"
                ? parameters.require_code_owner_review
                : undefined);
            lastPushValues.push(typeof parameters.require_last_push_approval === "boolean"
                ? parameters.require_last_push_approval
                : undefined);
            const threadResolution = parameters.required_review_thread_resolution ?? parameters.require_thread_resolution;
            threadValues.push(typeof threadResolution === "boolean" ? threadResolution : undefined);
            continue;
        }
        const checks = parameters.required_status_checks;
        upToDateValues.push(typeof parameters.strict_required_status_checks_policy === "boolean"
    ...
    if (relevantRules === 0)
        return { policy: null, malformed, count: rules.length };
    return {
        policy: {
            ...
            requiredStatusCheckApps: requiredCheckApps
                .filter((check, index, all) => all.findIndex((candidate) => candidate.context === check.context &&
                candidate.integrationId === check.integrationId) === index)
                .sort((left, right) => compareText(left.context, right.context) ||
...
function isRestPartial(response, returned, total) {
    return hasNextPage(response) || total > returned;
}
function unavailableWarning(label, error) {
    return `${label} were unavailable${httpSuffix(error)}`;
}
export async function collectPullRequestFlight(reference, options = {}) {
    ...
    let pullResponse;
    try {
        pullResponse = await client.request("GET /repos/{owner}/{repo}/pulls/{pull_number}", parameters);
    }
    catch (error) {
    ...
    if (!subject)
        throw new Error("GitHub pull request metadata had an unexpected shape");
    collection.push(safeSource("pull_request", "available"));
    let reviewThreads = null;
    if (!options.token) {
        collection.push(safeSource("graphql", "not_attempted", "Authentication is required for review-decision and review-thread evidence."));
    }
    ...
    try {
        ...
        if (!parsed) {
            ...
            const partial = parsed.malformed || isRestPartial(response, parsed.values.length, parsed.total);
            if (partial) {
    ...
    let workflowRuns = [];
    let workflowSourceStatus = "available";
    let workflowSourceDetail;
    try {
        ...
        if (!parsed) {
            ...
            for (const run of actionRequired.slice(0, MAX_ACTION_REQUIRED_JOB_LOOKUPS)) {
                try {
                    const jobsResponse = await client.request("GET /repos/{owner}/{repo}/actions/runs/{run_id}/jobs", {
                        owner: reference.owner,
                        repo: reference.repo,
                        run_id: run.id,
                        per_page: 100,
    ...
    try {
        ...
        if (!parsed) {
            ...
            if (partial) {
                ...
                const detail = parsed.policy === null
