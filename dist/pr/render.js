...
export function renderPullRequestFlight(report) {
    const actors = report.outcome.nextActors.length > 0 ? report.outcome.nextActors.join(", ") : "none";
    const visibleSignals = report.signals.filter((signal) => (signal.status !== "pass" && signal.status !== "info") ||
    ...
    if (signalLines.length === 0) {
        ...
        if (pullRequestUrl)
            signalLines.push(row("SOURCE", pullRequestUrl));
    }
    const warningLines = report.collection.warnings.map((warning) => `- ${sanitizeText(warning, 240)}`);
    return `${[
