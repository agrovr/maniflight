...
function applyWaivers(checks, ignoreRules) {
    return checks.map((check) => {
        const waiver = ignoreRules.find((candidate) => {
            if (candidate.rule !== check.ruleId)
                return false;
...
export async function runManiflight(options = {}) {
    const root = resolve(options.root ?? process.cwd());
    const loaded = await loadConfig(root, options.configPath);
    const filesystem = await collectFilesystem(root, loaded.config);
    const workflowCollection = await collectWorkflows(filesystem.root, filesystem.files, loaded.config, filesystem.collection.parsedBytes);
    let github;
