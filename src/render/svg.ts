import type { Domain, ManiflightReport } from "../model.js";
...
export const DOMAIN_LABELS: Readonly<Record<RenderDomain, string>> = {
...
function domainState(report: ManiflightReport, domain: RenderDomain): string {
...
export function renderConstellationSvg(): string {
  return `
 ...
 */
export function renderReportSvg(report: ManiflightReport): string {
  const repositoryName = escapeXml(
  ...
  const domainStates = RENDER_DOMAINS.map(
    (domain) => `${DOMAIN_LABELS[domain]}: ${stateLabel(domainState(report, domain))}`,
  ).join("; ");
