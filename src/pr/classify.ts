...
const SIGNAL_STATUS_ORDER: Record<SignalStatus, number> = {
  blocked: 0,
  action_required: 1,
  waiting: 2,
  unknown: 3,
...
function signal(input: SignalInput): FlightSignal {
  return {
    id: input.id,
    status: input.status,
    actor: input.actor,
...
function evidence(label: string, url: string): EvidenceLink[] {
...
function checkSignal(check: CheckRunFact, policy: BranchPolicyFact | null): FlightSignal {
...
function statusSignal(status: CommitStatusFact, policy: BranchPolicyFact | null): FlightSignal {
  ...
  const actor: FlightActor = isKnownContributorGate ? "contributor" : "unknown";
  const confidence: ActorConfidence = isKnownContributorGate ? "inferred" : "unknown";
...
function hiddenActionRequiredWorkflowSignal(
  workflow: WorkflowRunFact,
  isFork: boolean,
): FlightSignal {
...
function outcomeSummary(status: FlightOutcome, subject: PullRequestFlightFacts["subject"]): string {
  const summaries: Record<FlightOutcome, string> = {
...
export function classifyPullRequestFlight(
  facts: PullRequestFlightFacts,
  observedAt: string,
): PullRequestFlightReport {
  const { subject } = facts;
  ...
  if (!subject.merged && subject.state === "open") {
    ...
    if (conflict) {
      ...
      signals.push(
        signal({
          ...
          detail:
            "The pull request is mergeable, but non-required checks may still need attention.",
          evidence: prEvidence,
        }),
      );
    } else {
    ...
    const latestDecisiveReviews = new Map<string, string>();
    ...
    const approvals = [...latestDecisiveReviews.values()].filter(
      (state) => state === "APPROVED",
    ).length;
    const requiredApprovals = facts.branchPolicy?.requiredApprovals;
    const reviewsComplete =
      facts.collection.find((source) => source.id === "reviews")?.status === "available";
    if (decision === null && hasChangesRequested) {
      signals.push(
        signal({
          ...
          detail:
            "GitHub did not confirm whether this review is still an active merge requirement.",
          evidence: prEvidence,
        }),
      );
    }
    ...
    if (decision === "CHANGES_REQUESTED") {
      ...
      signals.push(
        signal({
          ...
          blocking: true,
          summary:
            typeof requiredApprovals === "number" && requiredApprovals > 0
      ...
      decision === null &&
      facts.collection.every(
        (source) =>
          !["graphql", "reviews", "branch_rules"].includes(source.id) ||
    ...
    if (facts.reviewThreads) {
      ...
      const threadsComplete =
        facts.collection.find((source) => source.id === "graphql")?.status === "available";
      signals.push(
        signal({
          id: "review/threads",
          status:
            unresolved > 0 ? (required ? "blocked" : "info") : threadsComplete ? "pass" : "unknown",
          ...
          ...(unresolved > 0 && !required
            ? {
                detail:
                  "The active branch rules did not identify conversation resolution as required.",
              }
            : {}),
          evidence: prEvidence,
        }),
    ...
    const hiddenActionRequiredRuns = facts.workflowRuns.filter(
      (run) => (run.conclusion ?? run.status).toLowerCase() === "action_required" && run.jobs === 0,
    );
    signals.push(
      ...hiddenActionRequiredRuns.map((run) => hiddenActionRequiredWorkflowSignal(run, isFork)),
    );

    const requiredChecks =
      facts.branchPolicy?.requiredStatusCheckApps ??
      (facts.branchPolicy?.requiredStatusChecks ?? []).map((context) => ({
        context,
        integrationId: null,
      }));
    const checkRunsComplete =
      facts.collection.find((source) => source.id === "check_runs")?.status === "available";
    const commitStatusesComplete =
      facts.collection.find((source) => source.id === "commit_statuses")?.status === "available";
    for (const requirement of requiredChecks) {
      const observed =
        facts.checkRuns.some(
          (check) =>
      ...
      if (!observed) {
        const evidenceComplete =
          checkRunsComplete && (requirement.integrationId !== null || commitStatusesComplete);
        signals.push(
          signal({
    ...
    const modeledBlocker = signals.some(
      (value) => value.blocking && value.status !== "pass" && value.status !== "info",
    );
    ...
    for (const source of facts.collection) {
      if (source.id === "pull_request" || source.status === "available") continue;
      signals.push(
        signal({
  ...
  signals.sort(
    (left, right) =>
      SIGNAL_STATUS_ORDER[left.status] - SIGNAL_STATUS_ORDER[right.status] ||
  ...
  let status: FlightOutcome;
  if (subject.merged) status = "merged";
  else if (subject.state === "closed") status = "closed";
  else if (
    signals.some(
      (value) =>
        value.blocking && (value.status === "blocked" || value.status === "action_required"),
    )
  ) {
    status = "blocked";
  } else if (counts.actionRequired > 0) status = "action_required";
  else if (counts.waiting > 0) status = "waiting";
  else if (counts.unknown > 0) status = "unknown";
  else if (signals.some((value) => value.status === "info")) status = "ready_with_warnings";
  else status = "ready";

  const nextActors = ACTOR_ORDER.filter((actor) =>
    signals.some(
      (value) =>
        value.actor === actor &&
        value.status !== "pass" &&
  ...
  const nextActions = signals
    .map(actionForSignal)
    .filter((value): value is FlightAction => value !== null)
    .filter(
      (value, index, all) =>
        all.findIndex(
          (candidate) => candidate.actor === value.actor && candidate.summary === value.summary,
        ) === index,
    );
