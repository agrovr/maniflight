...
function earnedFactor(check: CheckResult): number {
  if (check.status === "pass") return 1;
  if (check.status === "warn") return 0.5;
  return 0;
}

export function scoreDomain(domain: Domain, checks: CheckResult[]): DomainResult {
  ...
  const earnedWeight = evaluatedChecks.reduce(
    (total, check) => total + check.weight * earnedFactor(check),
    0,
  );
...
export function scoreOverall(domains: Record<Domain, DomainResult>): OverallResult {
  ...
  const confidenceWeight = confidenceDomains.reduce(
    (total, domain) => total + DOMAIN_WEIGHTS[domain],
    0,
  );
  const score =
    scoredWeight === 0
      ? null
      : round(
          scoredDomains.reduce(
            (total, domain) => total + (domains[domain].score ?? 0) * DOMAIN_WEIGHTS[domain],
            0,
          ) / scoredWeight,
        );
  const confidence =
    confidenceWeight === 0
      ? 0
      : round(
          confidenceDomains.reduce(
            (total, domain) => total + domains[domain].confidence * DOMAIN_WEIGHTS[domain],
            0,
          ) / confidenceWeight,
        );
...
export function buildReport(
  ...
  const domains = Object.fromEntries(
    DOMAINS.map((domain) => [domain, scoreDomain(domain, checks)]),
  ) as Record<Domain, DomainResult>;
  const summary = {
