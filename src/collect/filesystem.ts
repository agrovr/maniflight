...
import { DEFAULT_EXCLUDES, type ManiflightConfig } from "../config.js";
import type {
  EntrypointRecord,
  FileCategory,
  FileRecord,
...
function isTestPath(path: string): boolean {
  ...
  return (
    /(^|\/)(__tests__|tests?|specs?)(\/|$)/u.test(lower) || /\.(spec|test)\.[a-z0-9]+$/u.test(lower)
  );
}
...
function classifyFile(path: string): FileCategory {
  ...
  if (lower.startsWith(".github/workflows/") && /\.ya?ml$/u.test(lower)) return "workflow";
  if (isManifest(lower)) return "manifest";
  if (isTestPath(lower)) return "test";
  if (DOCUMENT_NAMES.has(name) || lower.startsWith("docs/")) return "documentation";
  if (SOURCE_EXTENSIONS.has(extname(lower))) return "source";
...
function manifestKind(path: string): ManifestKind {
  const name = basename(path).toLowerCase();
...
function collectEntrypointValues(
  value: unknown,
  prefix: string,
...
function normalizeEntrypointTarget(target: string): string {
  const withoutPrefix = target.startsWith("./") ? target.slice(2) : target;
  return toPosix(withoutPrefix);
}

async function entrypointExists(root: string, repositoryPath: string): Promise<boolean> {
...
async function parseNpmManifest(
  root: string,
  path: string,
  source: string,
  ...
  try {
    ...
    for (const field of ["main", "module", "types"] as const) {
      if (typeof parsed[field] === "string") targets.set(field, parsed[field]);
    }
    if (typeof parsed.bin === "string") targets.set("bin", parsed.bin);
    else if (parsed.bin && typeof parsed.bin === "object" && !Array.isArray(parsed.bin)) {
      for (const [name, target] of Object.entries(parsed.bin)) {
        if (typeof target === "string") targets.set(`bin.${name}`, target);
      }
    }
    collectEntrypointValues(parsed.exports, "exports", targets);

    const manifestDirectory = toPosix(dirname(path));
    const entrypoints: EntrypointRecord[] = await Promise.all(
      [...targets.entries()]
...
function simpleManifest(path: string): ManifestRecord {
  return {
    path,
...
async function readBoundedText(
  ...
  if (file.size > config.limits.maxFileBytes) return undefined;
...
export async function collectFilesystem(
  root: string,
  config: ManiflightConfig,
): Promise<FilesystemCollection> {
  const canonicalRoot = await realpath(root);
  const rootMetadata = await lstat(canonicalRoot);
  if (!rootMetadata.isDirectory()) throw new Error("Repository path must be a directory");
  ...
  async function walk(directory: string): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (metadata.size > config.limits.maxFileBytes) skippedLargeFiles.push(repositoryPath);
  ...
  for (const file of manifestFiles) {
    ...
    manifests.push(
      basename(file.path).toLowerCase() === "package.json"
  ...
  let typescriptStrict: boolean | undefined;
  if (
    tsconfig &&
  ...
  return {
    ...
    facts: {
      ...
      issueTemplatePaths: files
        .filter(
          (file) =>
            file.path.toLowerCase().startsWith(".github/issue_template/") &&
