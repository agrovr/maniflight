...
const UNTRUSTED_CONTEXT =
  /(?:^|\s)(?:github\.(?:head_ref|ref|ref_name)|github\.event\.[\w.-]*(?:body|default_branch|email|head_ref|label|message|name|page_name|ref|title))(?:\s|$|[|&=!<>])/iu;
...
function parsePermissions(value: unknown): WorkflowPermissions | undefined {
  ...
  for (const [scope, level] of Object.entries(value)) {
    if (level === "read" || level === "write" || level === "none") permissions[scope] = level;
  }
  return permissions;
}

function parseActionReference(value: string): ActionReference {
  if (value.startsWith("./")) {
...
function detectQualitySignals(script: string): string[] {
  const lower = script.toLowerCase();
  const signals = new Set<string>();
  if (
    /\b(npm|pnpm|yarn|bun)\s+(run\s+)?test\b|\b(pytest|go test|cargo test|dotnet test|mvn test|gradle test)\b/u.test(
      lower,
    )
  ) {
    signals.add("test");
  ...
  if (
    /\b(npm|pnpm|yarn|bun)\s+(run\s+)?lint\b|\b(eslint|biome check|ruff check|golangci-lint)\b/u.test(
      lower,
    )
  ) {
    signals.add("lint");
  ...
  if (
    /\b(npm|pnpm|yarn|bun)\s+(run\s+)?build\b|\b(go build|cargo build|dotnet build|mvn package|gradle build)\b/u.test(
      lower,
    )
  ) {
    signals.add("build");
  ...
  if (
    /\b(npm|pnpm|yarn|bun)\s+(run\s+)?(typecheck|type-check|check:types)\b|\btsc\s+--noemit\b/u.test(
      lower,
    )
  ) {
  ...
  if (
    /\b(deploy|kubectl\s+apply|helm\s+upgrade|terraform\s+apply|vercel\s+deploy)\b/u.test(lower)
  ) {
    signals.add("deploy");
  }
  return [...signals].sort();
...
function collectUntrustedExpressions(
  script: string,
  path: string,
  job: string,
  step: number,
): UntrustedExpression[] {
  const output: UntrustedExpression[] = [];
...
function parseJobs(value: unknown, workflowPath: string): WorkflowJob[] {
  if (!isRecord(value)) return [];
  const jobs: WorkflowJob[] = [];

  for (const [jobId, jobValue] of Object.entries(value).sort(([left], [right]) =>
    left.localeCompare(right),
  )) {
    if (!isRecord(jobValue)) continue;
    const actionReferences: ActionReference[] = [];
    const qualitySignals = new Set<string>();
    const untrustedExpressions: UntrustedExpression[] = [];
    ...
    steps.forEach((stepValue, stepIndex) => {
      ...
      if (typeof stepValue.run === "string") {
        for (const signal of detectQualitySignals(stepValue.run)) qualitySignals.add(signal);
        untrustedExpressions.push(
          ...collectUntrustedExpressions(stepValue.run, workflowPath, jobId, stepIndex + 1),
        );
      }
    });

    const reusable =
      typeof jobValue.uses === "string" ? parseActionReference(jobValue.uses) : undefined;
    if (reusable) actionReferences.push(reusable);
    const permissions = parsePermissions(jobValue.permissions);
    const environment = parseEnvironment(jobValue.environment);
    const timeoutMinutes =
      typeof jobValue["timeout-minutes"] === "number" ? jobValue["timeout-minutes"] : undefined;
