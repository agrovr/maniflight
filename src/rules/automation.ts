import type { CheckResult, RepositorySnapshot, Rule, WorkflowJob } from "../model.js";

function allJobs(snapshot: RepositorySnapshot): WorkflowJob[] {
  return snapshot.workflows.flatMap((workflow) => workflow.jobs);
}

export const workflowPresent: Rule = (snapshot) => ({
...
export const pullRequestValidation: Rule = (snapshot) => {
  const pullRequestWorkflows = snapshot.workflows.filter(
    (workflow) => workflow.triggers.includes("pull_request") && !workflow.parseError,
  );
  const signals = new Set(
    pullRequestWorkflows.flatMap((workflow) => workflow.jobs.flatMap((job) => job.qualitySignals)),
  );
  const hasTest = signals.has("test");
  const hasSecondGate = ["lint", "build", "typecheck", "security"].some((signal) =>
    signals.has(signal),
  );
...
export const jobTimeout: Rule = (snapshot) => {
...
export const deploySafety: Rule = (snapshot) => {
  const deployments = snapshot.workflows.flatMap((workflow) =>
    workflow.jobs
      .filter((job) => job.qualitySignals.includes("deploy"))
      .map((job) => ({ workflow, job })),
  );
  const protectedDeployments = deployments.filter(
    ({ workflow, job }) => workflow.concurrency && job.environment,
  );
  return {
    ...
    evidence: deployments.map(({ workflow, job }) => ({
      ...
      message: `${job.id}: ${job.environment ? "environment set" : "no environment"}; ${
        workflow.concurrency ? "concurrency set" : "no concurrency"
      }`,
      path: workflow.path,
    })),
