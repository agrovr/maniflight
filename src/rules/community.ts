...
function localDocumentation(snapshot: RepositorySnapshot, names: string[]): string | undefined {
  const lowered = names.map((name) => name.toLowerCase());
  return snapshot.facts.documentationPaths.find((path) =>
    lowered.some((name) => path.toLowerCase().endsWith(name)),
  );
}

function communityFile(snapshot: RepositorySnapshot, names: string[]): string | undefined {
  return (
    localDocumentation(snapshot, names) ??
...
export const readme: Rule = (snapshot) =>
  fileRule(snapshot, {
    id: "community/readme",
    title: "README",
...
export const license: Rule = (snapshot) =>
  fileRule(snapshot, {
...
export const contributing: Rule = (snapshot) =>
  fileRule(snapshot, {
...
export const codeOfConduct: Rule = (snapshot) =>
  fileRule(snapshot, {
...
export const issueTemplate: Rule = (snapshot) => ({
  ...
  weight: 1,
  evidence:
    snapshot.facts.issueTemplatePaths.length > 0
...
export const pullRequestTemplate: Rule = (snapshot) => ({
  ...
  weight: 1,
  evidence:
    snapshot.facts.pullRequestTemplatePaths.length > 0
...
export const supportChannel: Rule = (snapshot) => {
...
export const repositoryMetadata: Rule = (snapshot) => {
  if (!snapshot.github) {
    return {
...
export const communityRules: Rule[] = [
  readme,
  license,
  contributing,
  codeOfConduct,
